  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-10">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-4xl font-semibold text-navy-900">My listings</h1>
          <p className="mt-1 text-slate/60">Create, edit, and remove the properties you own.</p>
        </div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-3">
          {stats.map(({ icon: Icon, label, value }) => (
            <div key={label} className="flex items-center gap-4 rounded-2xl border border-line bg-white p-5">
              <span className="grid h-12 w-12 place-items-center rounded-2xl bg-navy-50 text-navy-600">
                <Icon className="h-6 w-6" />
              </span>
              <div>
                <p className="text-sm text-slate/60">{label}</p>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-navy-900/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-3xl border border-line bg-canvas p-6 shadow-soft animate-fade-up">
            <h3 className="font-display text-xl font-semibold text-navy-900">Delete this listing?</h3>
