...
export default function PropertyDetail() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-8">
      <button onClick={() => navigate(-1)} className="mb-5 flex items-center gap-1 text-sm font-semibold text-navy-600 hover:text-navy-700">
      ...
      <div className="grid gap-8 lg:grid-cols-[1.4fr_1fr]">
              ...
              {property.images.map((img, i) => (
                <button
                  ...
                  className={`h-20 w-28 shrink-0 overflow-hidden rounded-2xl border-2 transition ${
                    active === i ? 'border-navy-500' : 'border-transparent opacity-70 hover:opacity-100'
                  }`}
                >
        ...
        <div>
          <div className="flex flex-wrap gap-2">
            ...
            <span className="chip bg-navy-800 text-white">{property.propertyType}</span>
          </div>

          <h1 className="mt-3 font-display text-3xl font-semibold text-navy-900">{property.title}</h1>
          <p className="mt-2 flex items-center gap-1.5 text-slate/60">
          ...
          <div className="mt-5 flex gap-6 rounded-2xl border border-line bg-white p-4">
            <span className="flex items-center gap-2 text-slate/70">
              <BedDouble className="h-5 w-5 text-navy-500" /> {property.bedrooms} bedrooms
            </span>
            <span className="flex items-center gap-2 text-slate/70">
              <Bath className="h-5 w-5 text-navy-500" /> {property.bathrooms} bathrooms
            </span>
          </div>

          <div className="mt-5">
            <h2 className="font-display text-lg font-semibold text-navy-800">About this home</h2>
            <p className="mt-2 whitespace-pre-line text-slate/70">{property.description}</p>
          </div>

          {/* Owner / contact */}
          <div className="mt-6 rounded-2xl border border-line bg-navy-50/40 p-4">
            <p className="text-sm font-semibold text-navy-800">Listed by</p>
            <div className="mt-2 flex items-center gap-3">
              {property.owner?.avatar ? (
                <img src={property.owner.avatar} alt="" className="h-10 w-10 rounded-full object-cover" />
              ) : (
                <span className="grid h-10 w-10 place-items-center rounded-full bg-navy-800 text-white">
                  {property.owner?.username?.[0]?.toUpperCase()}
                </span>
              )}
              <div>
                <p className="font-semibold text-navy-900">{property.owner?.username}</p>
                {property.owner?.email && (
                  <p className="flex items-center gap-1 text-sm text-slate/60">
