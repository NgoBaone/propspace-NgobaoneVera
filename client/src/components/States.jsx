...
export const Loading = ({ label = 'Loading listings…' }) => (
  <div className="flex flex-col items-center justify-center gap-3 py-20 text-navy-600">
...
export const Empty = ({ title = 'No listings yet', message, action }) => (
  <div className="flex flex-col items-center justify-center gap-3 rounded-3xl border border-dashed border-navy-200 bg-white/60 py-20 text-center">
    <SearchX className="h-10 w-10 text-navy-400" aria-hidden="true" />
    <h3 className="font-display text-xl text-navy-800">{title}</h3>
    {message && <p className="max-w-sm text-slate/60">{message}</p>}
    {action}
  </div>
);
