    ...
    <aside className="rounded-3xl border border-line bg-white p-5 shadow-card">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="flex items-center gap-2 font-display text-lg font-semibold text-navy-800">
          <SlidersHorizontal className="h-5 w-5 text-navy-500" /> Filters
        </h3>
        <button onClick={onReset} className="flex items-center gap-1 text-sm text-slate/50 hover:text-red-600">
            ...
            {PURPOSES.map((p) => (
              <button
                ...
                className={`flex-1 rounded-pill px-3 py-2 text-sm font-semibold transition ${
                  filters.purpose === p ? 'bg-navy-600 text-white' : 'bg-navy-50 text-navy-700'
                }`}
              >
                {p}
              </button>
